...
async def automod_settings(
    ...
    if spam_filter is not None:
        ...
        changes.append(f"Spam Filter: {'On' if spam_filter else 'Off'}")
    if caps_filter is not None:
        config["caps_filter"] = caps_filter
        changes.append(f"Caps Filter: {'On' if caps_filter else 'Off'}")
    if link_filter is not None:
        config["link_filter"] = link_filter
        changes.append(f"Link Filter: {'On' if link_filter else 'Off'}")
    if invite_filter is not None:
        config["invite_filter"] = invite_filter
        changes.append(f"Invite Filter: {'On' if invite_filter else 'Off'}")
    if profanity_filter is not None:
        config["profanity_filter"] = profanity_filter
        changes.append(f"Profanity Filter: {'On' if profanity_filter else 'Off'}")
    ...
    if warn_count >= config["max_warnings"]:
        try:
            ...
            await interaction.response.send_message(
                f"**{user.mention}** has been **AUTO-BANNED** for reaching {warn_count} warnings!\n"
